...
class Settings(BaseModel):
    ...
    max_runs_bytes: int = Field(default_factory=lambda: int(os.getenv("MAX_RUNS_BYTES", str(5 * 1024**3))))
    max_runs_count: int = Field(default_factory=lambda: int(os.getenv("MAX_RUNS_COUNT", "200")))
    max_upload_age_hours: int = Field(default_factory=lambda: int(os.getenv("MAX_UPLOAD_AGE_HOURS", "24")))

    # Logging
    log_max_bytes: int = Field(default_factory=lambda: int(os.getenv("LOG_MAX_BYTES", str(20 * 1024**2))))
    ...
    huey_fsync: bool = Field(default_factory=lambda: os.getenv("HUEY_FSYNC", "false").lower() == "true")
