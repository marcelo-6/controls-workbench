# backend/tests/unit/domain/test_jobs_service.py
...
- tool runners (pure-ish callables)
- job state transitions (SQLite jobs table)
- job events (SQLite events table)
- artifacts persistence (filesystem) + indexing (SQLite artifacts table)

Key invariants:
- A tool runner must not talk directly to SQLite or the filesystem beyond the
  ToolContext callbacks.
- JobsRepo is updated with durable progress/status fields for UI polling.
- Failures must set stable error fields (`error_code`, `error_message`, etc.).
"""
